...
const brands = [
  ...
  {
    name: "Quora", color: "#B92B27", icon: (
      <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
        <path d="M12.738 18.076c-.95-1.907-2.294-3.312-4.237-3.312-.334 0-.636.044-.921.131l-.655-1.287c.894-.9 2.244-1.413 3.883-1.413 2.084 0 3.588.682 4.787 2.088.319-1.006.478-2.168.478-3.514 0-5.123-2.306-8.076-6.26-8.076-3.941 0-6.263 2.953-6.263 8.076 0 5.122 2.322 8.072 6.263 8.072.744 0 1.416-.1 2.017-.288l.908 1.523zm-.882 2.627l-1.238-2.037c-.616.131-1.268.198-1.952.198-5.086 0-8.666-3.973-8.666-10.095C0 3.655 3.58-.003 8.666-.003c5.1 0 8.668 3.658 8.668 10.772 0 2.902-.693 5.308-1.94 7.043.555.689 1.18 1.166 1.955 1.166.655 0 1.078-.273 1.373-.786l.867 1.024c-.564.824-1.444 1.413-2.778 1.413-1.576 0-2.773-.672-3.955-1.926z" />
      </svg>
    )
  },
  {
...
const LogoMarquee = () => {
  // Use all brands for density (DNA needs many points to look good)
  const dnaBrands = [...brands, ...brands];

  return (
    <div className="relative w-screen h-[500px] flex items-center justify-center overflow-visible">
      {/* Logos flowing in a DNA Helix */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        {dnaBrands.map((brand, i) => (
          <LogoHelixItem
            key={`${brand.name}-${i}`}
            brand={brand}
            index={i}
            total={dnaBrands.length}
            strand={i % 2} // 0 for Strand A, 1 for Strand B
          />
        ))}
      </div>
    </div>
...
const LogoHelixItem = ({ brand, index, total, strand }: { brand: typeof brands[0], index: number, total: number, strand: number }) => {
  ...
  useAnimationFrame((time) => {
    const speed = 0.00015;
    // Stagger based on index within the same strand
    const staggeredStart = ((index / 2) / (total / 2));
    const progress = ((time * speed) + staggeredStart) % 1;

    // Horizontal path across the screen
    const currentX = -500 + (progress * 1500);

    // DNA Helix Path
    // Strand A starts at top (cos(0)=1), Strand B at bottom (cos(PI)=-1)
    const frequency = Math.PI * 3; // Number of crossings
    const phase = strand === 0 ? 0 : Math.PI;
    const waveProgress = progress * frequency + phase;

    const amplitude = 160;
    const currentY = Math.cos(waveProgress) * amplitude;

    // 3D Depth Effect using Sine for Z-plane
    const depth = Math.sin(waveProgress); // -1 (back) to 1 (front)

    x.set(currentX);
    y.set(currentY);

    // Opacity: Fade in/out at edges + lower opacity when "behind"
    let baseOpacity = 1;
    if (progress < 0.1) baseOpacity = progress * 10;
    else if (progress > 0.85) baseOpacity = Math.max(0, (1 - progress) * 6.6);

    // Soften opacity for logos in the "back"
    opacity.set(baseOpacity * (0.6 + (depth + 1) * 0.2));

    // Scale: Larger when in "front", smaller when in "back"
    scale.set(0.7 + (depth + 1) * 0.25);

    // Set Z-Index for proper layering
    zIndex.set(Math.round((depth + 1) * 10));
  });

  return (
    <motion.div
      style={{ x, y, opacity, scale, zIndex: zIndex as any }}
      className="absolute flex items-center justify-center will-change-transform pointer-events-auto"
    >
      <div
        className="w-20 h-20 rounded-[2rem] bg-white dark:bg-slate-900 shadow-[0_15px_45px_rgba(0,0,0,0.06)] border border-black/[0.03] dark:border-white/[0.03] flex items-center justify-center transition-all duration-700 hover:scale-110 hover:shadow-2xl"
        style={{ color: brand.color }}
      >
        <div className="w-11 h-11 flex items-center justify-center filter drop-shadow-sm">
          {brand.icon}
        </div>
      </div>
    </motion.div>
